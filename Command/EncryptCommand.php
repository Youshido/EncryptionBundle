...
class EncryptCommand extends ContainerAwareCommand
{
    ...
    {
        $this
            ->setName('encryption:encrypt')
            ->addArgument('entity', InputArgument::REQUIRED)
            ->addArgument('field', InputArgument::REQUIRED)
            ->setDescription('Encrypt field for entity. This task must run only one time!');
    }
    ...
    private function encrypt($repository, $field)
    {
        ...
        foreach($items as $item){
            $encodedValue = $manager->encodeValue($item['field'], $key);
            ...
            $repository
                ...
                ->set('e.'.$field, ':value')
                ->where('e.id = :id')
                ->setParameter('value', $encodedValue)
